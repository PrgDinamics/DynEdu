...
export default function ProductsClient({ initialProductos }: Props) {
  ...
  // modal de confirmación (guardar / actualizar)
  ...
  const handleClickEliminar = async (p: Producto) => {
    const ok = window.confirm(
      `¿Seguro que deseas eliminar el producto ${p.internal_id} – ${p.descripcion}?`
    );
    if (!ok) return;
  ...
  return (
    <>
      ...
      {/* MODAL CONFIRMACIÓN GUARDAR / ACTUALIZAR */}
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button onClick={confirmarAccion} variant="contained" autoFocus>
            Aceptar
          </Button>
        </DialogActions>
      </Dialog>
